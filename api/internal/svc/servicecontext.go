...
import (
	...
	"tower/api/internal/config"
	svcsync "tower/api/internal/svc/sync"
	"tower/model"
	"tower/rpc/task/pb"
	"tower/scheduler"
...
func NewServiceContext(c config.Config) (*ServiceContext, error) {
	...
	if err := svcCtx.ensureDefaultAdmin(context.Background()); err != nil {
...
func (s *ServiceContext) ensureDefaultAdmin(ctx context.Context) error {
	user, err := s.UserModel.FindByUsername(ctx, "admin")
	if err != nil {
		return err
	}
	if user != nil {
	...
	workspaces, err := s.WorkspaceModel.Find(ctx, bson.M{}, 1, 1)
	if err != nil {
		return err
	}
	...
	workspaceId := ""
	if len(workspaces) > 0 {
		workspaceId = workspaces[0].Id.Hex()
	} else {
		workspace := &model.Workspace{
			Name:        "默认工作空间",
			Description: "系统默认工作空间",
		}
		if err := s.WorkspaceModel.Insert(ctx, workspace); err != nil {
			return err
		}
	...
	if err := s.UserModel.Insert(ctx, admin); err != nil {
		return err
	}

	logx.Infof("Default admin initialized: username=admin password=123456")
	return nil
}
