...
func WorkerHeartbeatHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Concurrency > 0 {
			workerKey := "tower:worker:" + req.WorkerName
...
func WorkerOfflineHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 删除Worker状态数据
		workerKey := fmt.Sprintf("tower:worker:%s", req.WorkerName)
		rdb.Del(r.Context(), workerKey)

		// 从Worker集合中移除
		rdb.SRem(r.Context(), "tower:workers", req.WorkerName)

		// 删除控制命令（如果有）
		controlKey := fmt.Sprintf("tower:worker:control:%s", req.WorkerName)
