...
func removeCronTaskFromRedis(ctx context.Context, svcCtx *svc.ServiceContext, cronTaskId string) {
	...
	// 删除运行次数记录
	runCountKey := fmt.Sprintf("tower:cron:runcount:%s", cronTaskId)
...
func CronTaskListHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, task := range tasks {
			...
			runCountKey := fmt.Sprintf("tower:cron:runcount:%s", task.CronTaskId)
...
func CronTaskSaveHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if isNew {
			...
			// 通知调度器重新加载
			svcCtx.RedisClient.Publish(ctx, "tower:cron:reload", cronTaskId)
		} else {
			// 更新 - 从MongoDB获取并更新
			...
			svcCtx.RedisClient.Publish(ctx, "tower:cron:reload", req.Id)
		}
...
func CronTaskDeleteHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 通知调度器移除任务
		svcCtx.RedisClient.Publish(ctx, "tower:cron:remove", req.Id)
...
func CronTaskBatchDeleteHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for _, id := range req.Ids {
			...
			svcCtx.RedisClient.Publish(ctx, "tower:cron:remove", id)
		}
...
func CronTaskRunNowHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// 通知调度器立即执行
		svcCtx.RedisClient.Publish(ctx, "tower:cron:runnow", req.Id)
