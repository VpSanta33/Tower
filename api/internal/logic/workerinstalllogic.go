...
func (l *WorkerInstallLogic) GetInstallCommand(req *types.WorkerInstallCommandReq) (*types.WorkerInstallCommandResp, error) {
	...
	// 获取或生成安装密钥
	installKeyKey := "tower:worker:install_key"
	installKey, err := rdb.Get(l.ctx, installKeyKey).Result()
	if err != nil && err != redis.Nil {
		l.Logger.Errorf("[WorkerInstall] Failed to read install key: %v", err)
		return nil, xerr.NewServerError("读取 Worker 安装密钥失败，请检查 Redis 连接")
	}
	if err == redis.Nil || installKey == "" {
		// 生成新的安装密钥
		installKey = generateInstallKey()
		// 保存到Redis，永不过期（可以通过重新生成来更新）
		if err := rdb.Set(l.ctx, installKeyKey, installKey, 0).Err(); err != nil {
			l.Logger.Errorf("[WorkerInstall] Failed to save install key: %v", err)
			return nil, xerr.NewServerError("保存 Worker 安装密钥失败，请检查 Redis 连接")
		}
	...
	commands["linux"] = fmt.Sprintf(
		`curl -fsSL http://%s/api/v1/worker/download?os=linux -o tower-worker && chmod +x tower-worker && ./tower-worker -k %s -s %s`,
		serverAddrClean, installKey, "http://"+serverAddrClean,
	)

	// Linux一键安装（后台运行）
	commands["linux_daemon"] = fmt.Sprintf(
		`curl -fsSL http://%s/api/v1/worker/download?os=linux -o tower-worker && chmod +x tower-worker && nohup ./tower-worker -k %s -s %s > /dev/null 2>&1 &`,
		serverAddrClean, installKey, "http://"+serverAddrClean,
	)

	// Windows安装命令（PowerShell）
	commands["windows"] = fmt.Sprintf(
		`Invoke-WebRequest -Uri "http://%s/api/v1/worker/download?os=windows" -OutFile tower-worker.exe; .\tower-worker.exe -k %s -s %s`,
		serverAddrClean, installKey, "http://"+serverAddrClean,
	)

	// Windows安装命令（CMD + certutil）
	commands["windows_cmd"] = fmt.Sprintf(
		`certutil -urlcache -split -f http://%s/api/v1/worker/download?os=windows tower-worker.exe && tower-worker.exe -k %s -s %s`,
		serverAddrClean, installKey, "http://"+serverAddrClean,
	)

	// // macOS安装命令
	// commands["darwin"] = fmt.Sprintf(
	// 	`curl -fsSL http://%s/api/v1/worker/download?os=darwin -o tower-worker && chmod +x tower-worker && ./tower-worker -k %s -s %s`,
...
func (l *WorkerInstallLogic) RefreshInstallKey() (*types.WorkerRefreshKeyResp, error) {
	...
	// 保存到Redis
	installKeyKey := "tower:worker:install_key"
	if err := rdb.Set(l.ctx, installKeyKey, newKey, 0).Err(); err != nil {
		l.Logger.Errorf("[WorkerInstall] Failed to refresh install key: %v", err)
		return nil, xerr.NewServerError("刷新 Worker 安装密钥失败，请检查 Redis 连接")
	}
...
func (l *WorkerInstallLogic) ValidateInstallKey(req *types.WorkerValidateKeyReq) (*types.WorkerValidateKeyResp, error) {
	...
	registerKey := fmt.Sprintf("tower:worker:register:%s", req.WorkerName)
...
func (l *WorkerInstallLogic) GetWorkerBinaryInfo(osType, arch string) (*types.WorkerBinaryInfoResp, error) {
	...
	// 构建文件名
	filename := "tower-worker"
	if osType == "windows" {
		filename = "tower-worker.exe"
	}
