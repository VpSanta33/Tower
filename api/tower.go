...
var configFile = flag.String("f", "etc/tower.yaml", "the config file")
...
func main() {
	...
	if c.Auth.AccessSecret == "" {
		logx.Error("JWT secret not configured. Set TOWER_JWT_SECRET environment variable.")
 ...
 | |________) | |__| | \  /\  /  | |\  |
  \_____|_____/ \____/   \/  \/   |_| \_| 
                                         `)
	fmt.Println("---------------------------------------------------------")
	logx.Info("Tower API Service Starting")
	...
	logx.Infof("Tower API is running at: %s:%d", c.Host, c.Port)
...
func startCronExecuteSubscriber(svcCtx *svc.ServiceContext, sched *scheduler.Scheduler) {
	...
	for {
		pubsub := svcCtx.RedisClient.Subscribe(ctx, "tower:cron:execute")
...
func createAndPushCronTask(ctx context.Context, svcCtx *svc.ServiceContext, sched *scheduler.Scheduler, msg *CronExecuteMessage) error {
	...
	// 保存主任务信息到 Redis
	taskInfoKey := "tower:task:info:" + newTaskId
	...
	for i, batch := range batches {
		...
		// 保存子任务信息到 Redis（多批次时）
		if len(batches) > 1 {
			subTaskInfoKey := "tower:task:info:" + subTaskId
