...
func NewScheduler(rdb *redis.Client) *Scheduler {
	return &Scheduler{
		...
		processingKey: "tower:task:processing",
		workerLoadKey: "tower:worker:load",
...
func (s *Scheduler) GetWorkerQueueKey(workerName string) string {
	return fmt.Sprintf("tower:task:queue:worker:%s", strings.ToLower(workerName))
}
...
func (s *Scheduler) GetCancelSignalKey(taskId string) string {
	return fmt.Sprintf("tower:task:cancel:%s", taskId)
}
...
func (s *Scheduler) PublishCancelSignal(ctx context.Context, taskId, action string) error {
	...
	// 发布到取消信号频道
	return s.rdb.Publish(ctx, "tower:task:cancel", data).Err()
}
...
func (s *Scheduler) SubscribeCancelSignals(ctx context.Context) <-chan *CancelSignal {
	...
	go func() {
		...
		pubsub := s.rdb.Subscribe(ctx, "tower:task:cancel")
		defer pubsub.Close()
