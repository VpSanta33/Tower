...
func (m *CronManager) executeTask(task *CronTask) {
	...
	// 增加运行次数
	runCountKey := fmt.Sprintf("tower:cron:runcount:%s", taskId)
	...
	m.rdb.Publish(ctx, "tower:cron:execute", string(cronExecData))
}
		...
		subscribeLoop:
			for {
				...
				case msg, ok := <-ch:
					...
					switch msg.Channel {
					case "tower:cron:reload":
						m.ReloadTask(ctx, msg.Payload)
					case "tower:cron:remove":
						m.mu.Lock()
