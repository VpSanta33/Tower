  ...
  <div class="dashboard-modern" :class="{ 'is-dark': themeStore.isDark }">
    <section class="dashboard-hero">
      <div class="hero-copy">
        ...
        <h1>资产安全态势总览</h1>
        <p>汇聚资产、任务、漏洞与 Worker 节点状态，持续观察企业暴露面变化。</p>
      </div>
...
const totalAssets = computed(() => stats.domains + stats.ips + stats.ports + stats.sites)
const riskState = computed(() => {
  if (stats.vulns === 0) return '稳态'
  if (stats.vulnCritical > 0 || stats.vulnHigh > 0) return '高关注'
  return '观察中'
})
const workerHealth = computed(() => {
  if (workerStats.online > 0) return `${workerStats.online} 在线`
  return '待接入'
})
...
.dashboard-hero {
  ...
  border-radius: 20px;
  background:
    linear-gradient(135deg, rgba(7, 17, 31, 0.95), rgba(15, 35, 64, 0.78)),
    radial-gradient(circle at 82% 16%, rgba(103, 232, 249, 0.28), transparent 36%);
  ...
  &::before {
    ...
    mask-image: radial-gradient(circle at 72% 38%, black, transparent 70%);
  }
  ...
  &::after {
    ...
    box-shadow: inset 0 0 50px rgba(56, 189, 248, 0.16);
  }
}
