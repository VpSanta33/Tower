...
warning() { echo -e "\033[33m[Tower]\033[0m $*"; }
error() { echo -e "\033[31m[Tower]\033[0m $*"; }
abort() { error "$*"; exit 1; }

# Ctrl+C 退出
trap 'echo ""; info "已退出 Tower 管理工具"; exit 0' INT

# 确认提示
confirm() {
    echo -e -n "\033[36m[Tower] $* \033[1;36m(Y/n)\033[0m "
    read -n 1 -s opt
    echo
...
get_local_version() {
    # 检查容器是否存在
    if ! docker inspect "tower_api" >/dev/null 2>&1; then
        echo "未安装"
        return
    fi
    ...
    # 尝试从容器镜像标签获取版本
    local image=$(docker inspect --format='{{.Config.Image}}' "tower_api" 2>/dev/null)
    if [ -n "$image" ]; then
    ...
    # 如果都获取不到，显示镜像创建日期
    local created=$(docker inspect --format='{{.Created}}' "tower_api" 2>/dev/null | cut -d'T' -f1)
    if [ -n "$created" ]; then
        echo "latest ($created)"
    else
        echo "unknown"
...
init_env_file() {
    if [ -f ".env" ]; then
        # 检查 .env 中是否已有 Tower_JWT_SECRET
        if grep -q "^Tower_JWT_SECRET=" .env 2>/dev/null; then
            info "检测到 .env 中已有 Tower_JWT_SECRET，跳过生成"
            return
        fi
        # 追加到已有 .env
        local secret=$(generate_jwt_secret)
        echo "" >> .env
        echo "# Tower JWT Secret (自动生成，请勿泄露)" >> .env
        echo "Tower_JWT_SECRET=${secret}" >> .env
        info "已在 .env 中追加 Tower_JWT_SECRET"
    else
        # 创建新的 .env
        local secret=$(generate_jwt_secret)
        echo "# Tower 环境配置 (自动生成，请勿提交到版本控制)" > .env
        echo "# JWT Secret: 用于签发和验证 JWT Token，更换后已登录用户需重新登录" >> .env
        echo "Tower_JWT_SECRET=${secret}" >> .env
        info "已创建 .env 并生成 Tower_JWT_SECRET"
    fi
}
...
install_tower() {
    ...
    if [ ! -f "$COMPOSE_FILE" ]; then
        abort "未找到 $COMPOSE_FILE，请确保在 Tower 目录下执行"
    fi
...
show_version() {
    ...
    for service in "${SERVICES[@]}"; do
        ...
        case $service in
            "tower-api") container_name="tower_api" ;;
            "tower-rpc") container_name="tower_rpc" ;;
            "tower-web") container_name="tower_web" ;;
        esac
...
wait_for_healthy() {
    ...
    while [ $elapsed -lt $timeout ]; do
        ...
        for service in "tower_api" "tower_rpc" "tower_web"; do
...
# 一键卸载
uninstall_tower() {
    warning "此操作将删除所有 Tower 容器和数据！"

    if ! confirm "确认卸载 Tower?"; then
        info "取消卸载"
        return
    fi
...
show_logs() {
    ...
    case $opt in
        1) docker logs -f --tail 100 tower_api ;;
        2) docker logs -f --tail 100 tower_rpc ;;
        3) docker logs -f --tail 100 tower_web ;;
...
main_menu() {
    ...
    echo ""
    echo -e "\033[33m===================================================="
    echo -e " Tower 管理工具 v${SCRIPT_VERSION}"
...
# 命令行参数支持
case "${1:-}" in
    install) check_docker; install_tower ;;
    upgrade) check_docker; upgrade_tower ;;
    uninstall) check_docker; uninstall_tower ;;
    status) check_docker; show_status ;;
    start) check_docker; start_tower ;;
    stop) check_docker; stop_tower ;;
    restart) check_docker; restart_tower ;;
